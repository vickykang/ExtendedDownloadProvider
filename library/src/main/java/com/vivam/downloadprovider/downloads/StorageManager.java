...
class StorageManager {
    ...
    private static final long sMaxdownloadDataDirSize = 200 * 1024 * 1024;
    ...
    void verifySpace(int destination, String path, long length) throws StopRequestException {
        ...
        switch (destination) {
            case Downloads.DESTINATION_CACHE_PARTITION:
            case Downloads.DESTINATION_CACHE_PARTITION_NOROAMING:
            case Downloads.DESTINATION_CACHE_PARTITION_PURGEABLE:
                dir = mDownloadDataDir;
                break;
            case Downloads.DESTINATION_EXTERNAL:
                dir = mExternalStorageDir;
                break;
            case Downloads.DESTINATION_SYSTEMCACHE_PARTITION:
                dir = mSystemCacheDir;
                break;
            case Downloads.DESTINATION_FILE_URI:
    ...
    private synchronized void findSpace(File root, long targetBytes, int destination)
        ...
        if (destination == Downloads.DESTINATION_FILE_URI ||
                destination == Downloads.DESTINATION_EXTERNAL) {
            if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                throw new StopRequestException(Downloads.STATUS_DEVICE_NOT_FOUND_ERROR,
        ...
        if (bytesAvailable < sDownloadDataDirLowSpaceThreshold) {
            ...
            if (bytesAvailable < sDownloadDataDirLowSpaceThreshold) {
                ...
                if (root.equals(mSystemCacheDir)) {
                    ...
                    throw new StopRequestException(Downloads.STATUS_INSUFFICIENT_SPACE_ERROR,
    ...
    private long discardPurgeableFiles(int destination, long targetBytes) {
        ...
        String destStr  = (destination == Downloads.DESTINATION_SYSTEMCACHE_PARTITION) ?
                String.valueOf(destination) :
                String.valueOf(Downloads.DESTINATION_CACHE_PARTITION_PURGEABLE);
        ...
        long totalFreed = 0;
        try {
            final int dataIndex = cursor.getColumnIndex(Downloads._DATA);
    ...
    private void trimDatabase() {
        ...
        Cursor cursor = null;
        try {
            cursor = mContext.getContentResolver().query(Downloads.ALL_DOWNLOADS_CONTENT_URI,
            ...
            if (cursor.moveToFirst()) {
                ...
                while (numDelete > 0) {
                    Uri downloadUri = ContentUris.withAppendedId(
                            Downloads.ALL_DOWNLOADS_CONTENT_URI, cursor.getLong(columnId));

...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    private void hideNotification(Context context, long id) {
        ...
        try {
            if (cursor.moveToFirst()) {
                status = getInt(cursor, Downloads.COLUMN_STATUS);
                visibility = getInt(cursor, Downloads.COLUMN_VISIBILITY);
            } else {
    ...
    private void sendNotificationClickedIntent(Context context, long[] ids) {
        ...
        final Uri uri = ContentUris.withAppendedId(
                Downloads.ALL_DOWNLOADS_CONTENT_URI, ids[0]);
        final Cursor cursor = context.getContentResolver().query(uri, null, null, null, null);
        try {
            if (cursor.moveToFirst()) {
                packageName = getString(cursor, Downloads.COLUMN_NOTIFICATION_PACKAGE);
                clazz = getString(cursor, Downloads.COLUMN_NOTIFICATION_CLASS);
                isPublicApi = getInt(cursor, Downloads.COLUMN_IS_PUBLIC_API) != 0;
            } else {
